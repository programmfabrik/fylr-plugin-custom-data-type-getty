...
function getNewCustomExpiresAt() {
  ...
  const customExpirationConfig = info?.config?.plugin?.['custom-data-type-getty']?.config?.update_getty?.custom_expires_days || 1
...
(() => {
  ...
  if (info?.config?.plugin?.['custom-data-type-getty']?.config?.update_getty?.restrict_time === true) {
    ...
    if (getty_config?.from_time !== false && getty_config?.to_time !== false) {
      ...
      if (!isInTimeRange(hour, getty_config.from_time, getty_config.to_time)) {
